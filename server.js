...
async function getAcompanhamentoPedido(lojaId, query) {
  ...
  return encontrados.map(p => {
    ...
    const arr        = Array.isArray(p.itens_comprados) ? p.itens_comprados
                     : (typeof p.itens_comprados === 'string' ? JSON.parse(p.itens_comprados) : []);
...
async function registrarVendaPDV(lojaId, pedido) {
  ...
  const payload = {
    ...
    cliente_info:     typeof pedido.cliente_info    === 'string' ? JSON.parse(pedido.cliente_info)    : (pedido.cliente_info    || {}),
    itens_comprados:  typeof pedido.itens_comprados === 'string' ? JSON.parse(pedido.itens_comprados) : (pedido.itens_comprados || []),
